...
AUFK_SCHEMA = T.StructType(
    [
        ...
        T.StructField(
            "ZZGLTRP_ORIG", T.StringType(), True
        ),  # Original Basic Finish Date
...
UNIFIED_SCHEMA = T.StructType(
    [
        ...
        T.StructField(
            "start_date_source", T.StringType(), True
        ),  # Assuming string for non-parsed date
        T.StructField("start_date", T.DateType(), True),  # Assuming parsed date
        T.StructField(
            "order_start_timestamp_source", T.StringType(), True
        ),  # Source as string
        ...
        T.StructField(
            "order_finish_timestamp_source", T.StringType(), True
        ),  # Source as string
        ...
        T.StructField(
            "actual_on_time_deviation", T.DoubleType(), True
        ),  # Numeric for deviation
        T.StructField(
            "late_delivery_bucket", T.StringType(), True
        ),  # String for category/bucket
        T.StructField("mto_vs_mts_flag", T.StringType(), True),
        T.StructField(
            "order_finish_timestamp", T.TimestampType(), True
        ),  # Parsed timestamp
        T.StructField(
            "order_start_timestamp", T.TimestampType(), True
        ),  # Parsed timestamp
    ]
)
